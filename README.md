...
The CMOS inverter is ideal for digital applications such as processors, memory, and logic circuits as it offers advantages like low static power consumption, high speed, and excellent noise margins.
...
V1=O V, V2=1.8 V, Period=40 ns, Delay time=0 ns, Fall time=1 ns, Rise time=1 ns and Pulse width=20 ns.
Vdc is 1.8 V To perform DC analysis we have to go to **Launch->ADE L->dc**,in dc, check save operating point and select component parameter as vpulse to do the dc sweep. The DC output comes out to be as shown in figure below:
...
It can be inferred that VOL = 14.725 uV from the above figure.
...
From the above figure, it can be noted that VIL = 388.27 mV and VIH = 869.28 mV.
...
From the above figure, Tplh(given by dx in the image)= 306.36 ps.
...
From the above figure, Tphl= 27.80 ps.

Propagation delay,Tp= (Tplh+Tphl)/2 = 167.08 ps.
...
Average Static power dissipation = (4.03 + 481.98) nW / 2 = 243.005 nW.

Average Dynamic power dissipation = (79.38 + 94.22) μW / 2 = 86.80 μW.
